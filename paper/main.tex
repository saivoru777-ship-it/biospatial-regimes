...
clustered regimes occupy distinct regions of descriptor space
...
that VMR fingerprints---while informative for distinguishing among
clustering-positive process families---do not distinguish sub-Poisson
spatial organization. This is an expected consequence of VMR measuring
count overdispersion, which is near 1 for all three regime types.
...
out-of-regime, indicating that they lie outside the simulated reference
distribution---possibly reflecting mechanism mixtures, multi-parameter
interactions, observation noise, or model misspecification.
\end{abstract}
...
The overall silhouette score across all seven regimes is 0.172,
dominated by the degeneracy of the null cloud where three regimes
overlap. Within the clustering manifold, branch-constrained,
hierarchical, compartmental, and clustered regimes are more
distinguishable (Fig.~\ref{fig:descriptor_distributions}), though the
low global silhouette underscores that VMR descriptors alone do not
cleanly separate all seven regime types.
...
\begin{figure}[htbp]
  ...
  \caption{KDE-based soft regime similarity profiles for all 72 empirical
  fingerprints. Each row shows the normalized density of the fingerprint
  under each regime's distribution in PCA space. These values reflect
  geometric proximity in descriptor space, not mechanism probabilities
  or mixture proportions (see Section~\ref{sec:discussion}).}
...
similarity framework (Fig.~\ref{fig:mixture_validation}). A 70\% layered +
...
This is an important negative result: KDE-based similarities in PCA space
should not be interpreted as mixture proportions or mechanism probabilities.
The failure is consistent with the null cloud degeneracy and with the
general limitation that KDEs fitted to single-mechanism distributions
cannot model the interpolation that mixtures create. Users of this
framework should treat the similarity profiles as broad descriptive
coordinates, not as quantitative mechanistic decompositions.
...
The central result is that VMR-derived descriptors define a structured
clustering manifold: process families that produce above-Poisson variance
(clustering, compartmentalization, hierarchical nesting, branch
confinement) occupy distinguishable regions of a 6D descriptor space
compressed to 3 PCs explaining 96.0\% of variance. VMR descriptors do
more than detect ``something non-random''; they discriminate among
specified clustering-positive process families. We note, however, that
discrimination among a fixed set of simulated alternatives is weaker
than mechanism identification, which would require excluding
...
44 of 72 empirical fingerprints (61\%) lie outside the simulated
reference distribution. This could reflect mechanism mixtures,
multi-parameter interactions not captured by single-axis sweeps,
observation noise or preprocessing differences, or genuine biological
complexity exceeding single-mechanism simulations. Without controlled
tests distinguishing these explanations, the out-of-regime rate should
be interpreted cautiously. The simulation-defined regime space provides
a coordinate system for locating empirical data, not a classification
scheme.
